...
The contrast with SIFT is clean and worth stating in one line:
\textbf{these methods infer the consideration process as latent
structure from what was chosen; SIFT observes it directly and prices it
in units of search cost.} Inference versus observation. Both are
legitimate, and the inference approach has the advantage of working on
data already collected --- but a latent screening rule cannot be
manipulated, so it cannot answer what happens when the analyst changes
what is easy to see.
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-warning-color!10!white, colframe=quarto-callout-warning-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{The \(\nu\) labelling is not standard across papers}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

Yavorsky et al. (2021) uses the opposite convention, with \(\nu_1\)
continuation and \(\nu_2\) selection. Readers moving between the two
literatures should check the labelling before comparing smoothing
parameters or reported diagnostics. The crosswalk in
Section~\ref{sec-appendix-notation} records both.
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-important-color!10!white, colframe=quarto-callout-important-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{The test is not a Wald test}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

Because conjoint sits at the \emph{boundary} rather than the interior of
the parameter space --- \(\cost \to 0\) corresponds to
\(\log \cost \to -\infty\), and to \(g(\cost) \to \infty\) under the
alternative parameterization --- the null is not an interior point and
the usual \(\chi^2\) reference distribution does not apply. The
restriction must be tested against observable implications or against a
simulated reference distribution. Section~\ref{sec-est-nesting} develops
this.
...
data and not from conjoint data, which is an asymmetry worth being
honest about: the richer instrument can diagnose whether the simpler one
was adequate, but not the reverse. The cost of finding out is the cost
of fielding SIFT.
...
\subsection{Do Survey Clicks Measure Anything
Real?}\label{sec-disc-clicks}
...
that are not interpretable in isolation, and it is defensible ground ---
but it should be stated before a reviewer states it, and it constrains
the claims a SIFT study should make.
...
We take the first route and treat the second as a robustness check, but
we note the tension honestly: a practice block assumes learning
completes quickly, and whether it does is testable by looking for trends
in click counts and search costs across task order.
Section~\ref{sec-analysis} reports that diagnostic. If search costs
decline systematically across tasks, the practice block was insufficient
and the learning model becomes necessary rather than optional.
...
Most honestly: conjoint is understood, defensible, cheap to estimate,
and supported by four decades of accumulated practice. SIFT is none of
those things yet. A method that is right in principle and unfamiliar in
practice is a real cost to a research buyer, and we do not think it is
served by pretending otherwise.
...
able to run it only at the population level. ``What fraction of
respondents search sequentially'' is a question we can answer and nobody
has.
